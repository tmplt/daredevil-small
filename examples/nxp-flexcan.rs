//! A direct C-to-Rust rewrite of the S32K144 FlexCAN example from NXP's S32 SDK. Sets up
//! communication over CAN0 and continuously transmits messages.
//!
...
unsafe fn main() -> ! {
    ...
    while p.SCG.sosccsr.read().lk().is_0() {}
    p.SCG.sosccsr.write(|w| w.bits(0x1));
    while p.SCG.sosccsr.read().soscvld().is_0() {}

    // SPLL_init_160Mhz
    while p.SCG.spllcsr.read().spllvld().is_0() {}
    ...
    while p.SCG.csr.read().scs().bits() != 6 {}
